...
DOC:  Example of a single state machine using the python-statemachine package
      Implements a mobile robot accepting a drive request and checking for an obstacle
      Cycles at one cycle per second unless events present

      Create a text representation and png graphich with plotsm.py -m drive -sm DriveBehavior

REQUIRES:  python-statemachine  (pip3 install python-statemachine)
